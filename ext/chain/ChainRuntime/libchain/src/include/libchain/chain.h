...
// Temp macro to do GCC hand-compiler easier.
#define COMMIT(func, ...) COMMIT_(func, ##__VA_ARGS__, 2, 1)
#define COMMIT_(func, i, n, ...) COMMIT##n(func, i)
#define COMMIT1(func, ...) \
	write_to_gbuf((uint8_t*)&_global_ ## func ## _bak, \
			(uint8_t*)&_global_ ## func, sizeof(_global_ ## func))
#define COMMIT2(func, i) \
	write_to_gbuf((uint8_t*)&_global_ ## func ## _bak[i], \
			(uint8_t*)&_global_ ## func[i], sizeof(_global_ ## func[0]))

#define PRIV(func, ...) PRIV_(func, ##__VA_ARGS__, 2, 1)
#define PRIV_(func, i, n, ...) PRIV##n(func, i)
#define PRIV1(func, ...) \
	GV(func ## _bak) = GV(func);
#define PRIV2(func, i) \
	GV(func ## _bak[i]) = GV(func[i]);

#define DY_PRIV(func, i) \
	if (_global_ ## func ## _isDirty[i] != _numBoots) PRIV(func, i);
#define DY_COMMIT(func, i) \
	if (_global_ ## func ## _isDirty[i] != _numBoots) {\
		COMMIT(func, i);\
		_global_ ## func ## _isDirty[i] = _numBoots;\
	}
