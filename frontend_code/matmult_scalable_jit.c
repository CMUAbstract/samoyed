...
__atomic__<s1_rs:2, s2_cs:2> safe_matmult(
	__output__[s1_rs*s2_cs*2] int* dst,
	__input__[s1_rs*S1_CS*2] int* src1,
	__input__[S2_RS*s2_cs*2] int* src2, unsigned s1_rs, unsigned s2_cs) {
	...
	s1_rs /= 2;
	s2_cs /= 2;
	safe_matmult(dst, src1, src2, s1_rs, s2_cs);
	safe_matmult(dst + s2_cs, src1, src2 + s2_cs, s1_rs, s2_cs);
	safe_matmult(dst + s1_rs*D_CS, src1 + s1_rs*S1_CS, src2, s1_rs, s2_cs);
	safe_matmult(dst + s1_rs*D_CS + s2_cs, src1 + s1_rs*S1_CS, src2 + s2_cs, s1_rs, s2_cs);
	} __scaling_rule__
}
/*
...
#else
			safe_matmult(*dst_nv, *src_nv, *src_nv, S1_RS, S2_CS);
