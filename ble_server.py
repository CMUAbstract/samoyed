			...
			if packet.blePacket is not None:
					...
					if len(payload) == 16:
						...
						if counter > prev_counter:
							if data == DISPENSER_ON:
								print "{0} dispenser on!!".format(str(datetime.datetime.now().time()))
							elif data == WATER_ON:
								print "{0} water on!!".format(str(datetime.datetime.now().time()))
							else:
								print "{0} quiet!!".format(str(datetime.datetime.now().time()))
							if counter == 0xFF:
								prev_counter = -1 # Todo. not optimal on overflow
							else:
