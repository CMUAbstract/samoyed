...
int _jit_safe_0(char * key, char * plaintext, unsigned int len)  {
...
void safe_aes_run(char* key, char* plaintext, unsigned len) {
		int success = 0;
	...
	undoLogPtr = undoLogPtr_tmp;
	undoLogCnt = undoLogCnt_tmp;
	success = _jit_safe_0(key, plaintext, len);
	PROTECT_END();
	if (!success) {
;

	safe_aes_run(key, plaintext, len >> 1);
	safe_aes_run(key, plaintext + (len >> 1), len >> 1);
...
int main()
{
	...
	P1OUT |= BIT3;
	while (1) {
		safe_aes_run((char *)_jit_test_key, (char *)_jit_test_plaintext, 16);
...
#else
			safe_aes_run(key, plaintext, DATA_SIZE);
#endif
		}
#ifdef LOGIC
...
void init()
{
	...
	PRINTF(".%x. %u %u %u %u %u\r\n", pc, _jit_0_len, undoLogCnt, undoLogCnt_tmp, _jit_bndMayNeedUpdate, _jit_no_progress);
#endif
}
