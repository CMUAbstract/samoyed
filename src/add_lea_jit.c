...
int _jit_safe_0(unsigned int len, int * src, int * dst)  {
...
void safe_copy_and_add(unsigned len, int* src, int* dst) {
	...
	undoLogPtr = undoLogPtr_tmp;
	undoLogCnt = undoLogCnt_tmp;
	success = _jit_safe_0(len, src, dst);
	PROTECT_END();
	if (!success) {
;
	if (len % 2 == 0) {
...
int main()
{
	...
	char _jit_test_dst[ 2 << 1];
	PRINTF("CNT: %u %u\r\n", energy_overflow, energy_counter);
	energy_counter = 0;
	energy_overflow = 0;
	P1OUT |= BIT3;
	while (1) {
		safe_copy_and_add(2, (int *)_jit_test_src, (int *)_jit_test_dst);
	...
	while (1) {
		...
		GPIO(PORT_DBG, OUT) &= ~BIT(PIN_AUX_2);
		//if (dst_nv[CONFIG_SAMPLE_SIZE - 1] != 7) {
		//if (dst_nv[9] != 20) {
		//	while (1);
		//}
#else
