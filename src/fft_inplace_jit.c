...
int _jit_safe_0(_q15 * in, size_t len)  {
	size_t len_bak = len;
...
;
	...
	__asm__ volatile ("MOVX.A %1, %0":"=m"(DMA1DA) : "m"(in));
	// size in word
	DMA1SZ = len << 1;
	DMA1CTL = (DMADT_1 | DMASRCINCR_3 | DMADSTINCR_3 | DMAEN | DMAREQ );
...
void safe_cmplx_fft(_q15* in, size_t len) {
		int success = 0;
	if (in == in) {
		if (len <= _jit_0_len) {
	...
	undoLogPtr = undoLogPtr_tmp;
	undoLogCnt = undoLogCnt_tmp;
	success = _jit_safe_0(in, len);
	PROTECT_END();
	if (!success) {
;

		// do two fft
		safe_cmplx_fft(in, len >> 1);
		safe_cmplx_fft(in + len, len >> 1);
    ...
    MPY32CTL0 = MPYFRAC | MPYDLYWRTEN;
		...
		for (unsigned i = 0; i < len; i += 2) {
			msp_cmplx_btfly_fixed_q15(&in[i], &in[i + len],
					twiddlePtr + i);
		}
#if defined(__MSP430_HAS_MPY32__)
    MPY32CTL0 = ui16MPYState;
...
// too lazy to make this recursive..
void safe_bitrev(_q15* in, size_t size) {
	IF_ATOMIC();
...
// too lazy to make this recursive..
void safe_split(_q15* in, size_t size) {
	IF_ATOMIC();
...
int main()
{
	...
	P1OUT |= BIT3;
	while (1) {
		safe_cmplx_fft((_q15 *)_jit_test_in, 2);
	...
	while (1) {
		...
		for (unsigned i = 0; i < FFT_ITER; ++i) {
			...
			// This calculates X(k)
			safe_cmplx_fft(source, SAMPLES >> 1);
