...
#define SAMPLES SAMPLE_SIZE
#define FFT_ITER 10

#define LOG(...)
__nv _q15 source[SAMPLE_SIZE] = {
#include "fft_source.txt"	
};
__nv _q15 buf0[SAMPLE_SIZE];
__nv _q15 buf1[SAMPLE_SIZE];
DSPLIB_DATA(fft_inout,MSP_ALIGN_FFT_Q15(SAMPLE_SIZE)) _q15 fft_inout[SAMPLE_SIZE];
 ...
 */
const _q15 samoyed_cmplx_twiddle_table_16_q15[DSPLIB_TABLE_OFFSET+16] = {
    0x0010, 0x0000,
 ...
 */
const _q15 samoyed_cmplx_twiddle_table_32_q15[DSPLIB_TABLE_OFFSET+32] = {
    0x0020, 0x0000,
 ...
 */
const _q15 samoyed_cmplx_twiddle_table_128_q15[DSPLIB_TABLE_OFFSET+128] = {
    0x0080, 0x0000,
 ...
 */
const _q15 samoyed_cmplx_twiddle_table_256_q15[DSPLIB_TABLE_OFFSET+256] = {
    0x0100, 0x0000,
 ...
 */
const _q15 samoyed_cmplx_twiddle_table_512_q15[DSPLIB_TABLE_OFFSET+512] = {
    0x0200, 0x0000,
 ...
 */
const _q15 samoyed_cmplx_twiddle_table_1024_q15[DSPLIB_TABLE_OFFSET+1024] = {
    0x0400, 0x0000,
 ...
 */
const _q15 samoyed_cmplx_twiddle_table_2048_q15[DSPLIB_TABLE_OFFSET+2048] = {
    0x0800, 0x0000,
 ...
 */
const _q15 samoyed_cmplx_twiddle_table_4096_q15[DSPLIB_TABLE_OFFSET+4096] = {
    0x1000, 0x0000,
...
/*
void safe_cmplx_fft(_q15* in, _q15* out, size_t len) {
	IF_ATOMIC(KNOB(len, SAMPLES));

	msp_cmplx_fft_q15_params paramsCmplxFFT;

	paramsCmplxFFT.length = len;
	paramsCmplxFFT.bitReverse = false;
	paramsCmplxFFT.twiddleTable = samoyed_cmplx_twiddle_table_256_q15;

	// copy using dma
	__asm__ volatile ("MOVX.A %1, %0":"=m"(DMA1SA) : "m"(in));
	__asm__ volatile ("MOVX.A %1, %0":"=m"(DMA1DA) : "i"(fft_inout));
	// size in word
	DMA1SZ = len << 1;
	...
	msp_cmplx_fft_fixed_q15(&paramsCmplxFFT, fft_inout);

	// copy using dma
	__asm__ volatile ("MOVX.A %1, %0":"=m"(DMA1SA) : "i"(fft_inout));
	__asm__ volatile ("MOVX.A %1, %0":"=m"(DMA1DA) : "m"(out));
	// size in word
	DMA1SZ = len << 1;
	...
	ELSE();
		...
		// do two fft
		safe_cmplx_fft(in, out, len >> 1);
		safe_cmplx_fft(in + len, out + len, len >> 1);
...
#if SAMPLE_SIZE >= 32
			case 16:
				twiddlePtr = &samoyed_cmplx_twiddle_table_16_q15[DSPLIB_TABLE_OFFSET];
				break;
#endif
#if SAMPLE_SIZE >= 64
			case 32:
				twiddlePtr = &samoyed_cmplx_twiddle_table_32_q15[DSPLIB_TABLE_OFFSET];
				break;
#endif
#if SAMPLE_SIZE >= 128
			case 64:
				twiddlePtr = &samoyed_cmplx_twiddle_table_64_q15[DSPLIB_TABLE_OFFSET];
				break;
#endif
#if SAMPLE_SIZE >= 256
			case 128:
				twiddlePtr = &samoyed_cmplx_twiddle_table_128_q15[DSPLIB_TABLE_OFFSET];
				break;
#endif
#if SAMPLE_SIZE >= 512
			case 256:
				twiddlePtr = &samoyed_cmplx_twiddle_table_256_q15[DSPLIB_TABLE_OFFSET];
				break;
#endif
#if SAMPLE_SIZE >= 1024
			case 512:
				twiddlePtr = &samoyed_cmplx_twiddle_table_512_q15[DSPLIB_TABLE_OFFSET];
				break;
#endif
#if SAMPLE_SIZE >= 2048
			case 1024:
				twiddlePtr = &samoyed_cmplx_twiddle_table_1024_q15[DSPLIB_TABLE_OFFSET];
				break;
#endif
#if SAMPLE_SIZE >= 4096
			case 2048:
				twiddlePtr = &samoyed_cmplx_twiddle_table_2048_q15[DSPLIB_TABLE_OFFSET];
				break;
#endif
#if SAMPLE_SIZE >= 8192
			case 4096:
				twiddlePtr = &samoyed_cmplx_twiddle_table_4096_q15[DSPLIB_TABLE_OFFSET];
				break;
#endif
		}
    ...
    MPY32CTL0 = MPYFRAC | MPYDLYWRTEN;
		...
		for (unsigned i = 0; i < len; i += 2) {
			msp_cmplx_btfly_fixed_q15(&out[i], &out[i + len],
					twiddlePtr + i);
		}
#if defined(__MSP430_HAS_MPY32__)
    MPY32CTL0 = ui16MPYState;
...
void safe_memcpy(uint8_t* dst, uint8_t* src, size_t size) {
	IF_ATOMIC(KNOB(size, SAMPLES << 1));
	...
	safe_memcpy(dst, src, size >> 1);
	safe_memcpy(dst + (size >> 1), src + (size >> 1), size >> 1);

	END_IF();
}

// too lazy to make this recursive..
void safe_bitrev(_q15* in, _q15* out, size_t size) {
	IF_ATOMIC();
	...
	msp_cmplx_bitrev_q15_params paramsBitRev;
	paramsBitRev.length = size >> 1;
	msp_cmplx_bitrev_q15(&paramsBitRev, fft_inout);
...
// too lazy to make this recursive..
void safe_split(_q15* in, _q15* out, size_t size) {
	IF_ATOMIC();

	msp_split_q15_params paramsSplit;           // Split operation params
	paramsSplit.length = size;
	paramsSplit.twiddleTable = msp_cmplx_twiddle_table_256_q15;
		// set twiddleTable
		switch (size) {
#if SAMPLE_SIZE >= 16
			case 16:
				paramsSplit.twiddleTable = &samoyed_cmplx_twiddle_table_16_q15[DSPLIB_TABLE_OFFSET];
				break;
#endif
#if SAMPLE_SIZE >= 32
			case 32:
				paramsSplit.twiddleTable = &samoyed_cmplx_twiddle_table_32_q15[DSPLIB_TABLE_OFFSET];
				break;
#endif
#if SAMPLE_SIZE >= 64
			case 64:
				paramsSplit.twiddleTable = &samoyed_cmplx_twiddle_table_64_q15[DSPLIB_TABLE_OFFSET];
				break;
#endif
#if SAMPLE_SIZE >= 128
			case 128:
				paramsSplit.twiddleTable = &samoyed_cmplx_twiddle_table_128_q15[DSPLIB_TABLE_OFFSET];
				break;
#endif
#if SAMPLE_SIZE >= 256
			case 256:
				paramsSplit.twiddleTable = &samoyed_cmplx_twiddle_table_256_q15[DSPLIB_TABLE_OFFSET];
				break;
#endif
#if SAMPLE_SIZE >= 512
			case 512:
				paramsSplit.twiddleTable = &samoyed_cmplx_twiddle_table_512_q15[DSPLIB_TABLE_OFFSET];
				break;
#endif
#if SAMPLE_SIZE >= 1024
			case 1024:
				paramsSplit.twiddleTable = &samoyed_cmplx_twiddle_table_1024_q15[DSPLIB_TABLE_OFFSET];
				break;
#endif
#if SAMPLE_SIZE >= 2048
			case 2048:
				paramsSplit.twiddleTable = &samoyed_cmplx_twiddle_table_2048_q15[DSPLIB_TABLE_OFFSET];
				break;
#endif
#if SAMPLE_SIZE >= 4096
			case 4096:
				paramsSplit.twiddleTable = &samoyed_cmplx_twiddle_table_4096_q15[DSPLIB_TABLE_OFFSET];
				break;
#endif
		}
	...
	msp_split_q15(&paramsSplit, fft_inout);
...
__nv size_t _jit_0_len = 65535;
int _jit_safe_0(_q15 * in, _q15 * out, size_t len)  {
	size_t len_bak = len;
...
void safe_cmplx_fft(_q15* in, _q15* out, size_t len) {
	...
	undoLogPtr = undoLogPtr_tmp;
	undoLogCnt = undoLogCnt_tmp;
	success = _jit_safe_0(in, out, len);
	PROTECT_END();
	if (!success) {
;
...
// too lazy to make this recursive..
int _jit_safe_1(_q15 * in, _q15 * out)  {
;
	;
	;
...
void safe_bitrev(_q15* in, _q15* out) {
		if (in == out) {
		recursiveUndoLog(in,  SAMPLES << 1);
	}
	undoLogPtr = undoLogPtr_tmp;
	undoLogCnt = undoLogCnt_tmp;
	PROTECT_BEGIN();
...
// too lazy to make this recursive..
int _jit_safe_2(_q15 * in, _q15 * out)  {
;
	;
	;

	msp_split_q15_params paramsSplit;           // Split operation params
	paramsSplit.length = SAMPLES;
	paramsSplit.twiddleTable = msp_cmplx_twiddle_table_256_q15;
		// set twiddleTable
		switch (SAMPLES) {
#if SAMPLE_SIZE >= 16
...
void safe_split(_q15* in, _q15* out) {
		if (in == out) {
		recursiveUndoLog(in,  SAMPLES << 1);
	}
	undoLogPtr = undoLogPtr_tmp;
...
int main()
{
	...
	P1OUT &= ~BIT3;
	P1DIR |= BIT3;
	char _jit_test_in[ 2 << 2];
	fill_with_rand(_jit_test_in,  2 << 2);
	char _jit_test_out[ 2 << 2];
	PRINTF("CNT: %u %u\r\n", energy_overflow, energy_counter);
	energy_counter = 0;
	energy_overflow = 0;
	P1OUT |= BIT3;
	while (1) {
		safe_cmplx_fft((_q15 *)_jit_test_in, (_q15 *)_jit_test_out, 2);
	...
	P1OUT &= ~BIT3;
	P1DIR |= BIT3;
	char _jit_test_in[ SAMPLES << 1];
	fill_with_rand(_jit_test_in,  SAMPLES << 1);
	char _jit_test_out[ SAMPLES << 1];
	PRINTF("CNT: %u %u\r\n", energy_overflow, energy_counter);
	energy_counter = 0;
	energy_overflow = 0;
	P1OUT |= BIT3;
	while (1) {
		safe_bitrev((_q15 *)_jit_test_in, (_q15 *)_jit_test_out);
		if (energy_counter == 0xFFFF) {
			energy_overflow = 1;
		}
		else {
	...
	P1OUT |= BIT3;
	while (1) {
		safe_split((_q15 *)_jit_test_in, (_q15 *)_jit_test_out);
		if (energy_counter == 0xFFFF) {
			energy_overflow = 1;
		}
		else {
...
//	while (1) {
		...
		GPIO(PORT_DBG, OUT) &= ~BIT(PIN_AUX_0);
#endif
//		for (unsigned i = 0; i < FFT_ITER; ++i) {
			/* FFT starts from here */
#if QUICKRECALL == 1
			msp_fft_q15_params fftParams;
			fftParams.length = SAMPLES;
			fftParams.bitReverse = true;
#if SAMPLES == 1024
			fftParams.twiddleTable = samoyed_cmplx_twiddle_table_1024_q15;
#else
			fftParams.twiddleTable = samoyed_cmplx_twiddle_table_256_q15;
#endif

			sw_fft_fixed_q15(&fftParams, source);
#else

			/* 1. Manual bitreverse */
			//in: data, out: result
			safe_bitrev(source, buf0);

			/* 2. Complex FFT */
			// Assuming the Real sequence is a Real & Imaginary pair, compute FFT
			// Assuming the real signal is g(n),
			// We are assuming x(n) = g(2n) + i*g(2n+1)
			// This calculates X(k)
			safe_cmplx_fft(buf0, buf1, SAMPLES >> 1);

			/* 3. Split. */
			// This retrieves the actual G(k) from X(k)
			safe_split(buf1, buf0);
#endif
//		}
#ifdef LOGIC
