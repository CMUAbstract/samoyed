 ...
 */
msp_status tst_cmplx_fft_fixed_q15(const msp_cmplx_fft_q15_params *params, int16_t *src)
{
...
void safe_cmplx_fft(_q15* in, _q15* out, size_t len) {
	...
	paramsCmplxFFT.twiddleTable = 0; // only for LEA

	memcpy(fft_inout, in, len << 2);
	// len word goes in, len*2 word goes out (weird, but it is how it works)
	msp_cmplx_fft_fixed_q15(&paramsCmplxFFT, fft_inout);
	memcpy(out, fft_inout, len << 2);

	ELSE();

	// do two fft
	safe_cmplx_fft(in, out, len >> 1);
	// Be careful about the second out!! This is correct, because 
	// the first fft gets in of length len >> 1 and get out of length len
...
int main()
{
	...
	/* 1. Manual bitreverse */
	// This bitreverse the entire SAMPLES, not just half (even though .length is half)
	paramsBitRev.length = SAMPLES >> 1;
	status = msp_cmplx_bitrev_q15(&paramsBitRev, input);

	/* 2. Complex FFT */
	// Assuming the Real sequence is a Real & Imaginary pair, compute FFT
	// Assuming the real signal is g(n),
	// We are assuming a x(n) = g(2n) + i*g(2n+1)
	// This calculates X(k)
	//safe_cmplx_fft(data, result, SAMPLES >> 1);
	msp_cmplx_fft_q15_params paramsCmplxFFT;
	paramsCmplxFFT.length = SAMPLES >> 1;
	paramsCmplxFFT.bitReverse = false;
	paramsCmplxFFT.twiddleTable = samoyed_cmplx_twiddle_table_256_q15;
	status = tst_cmplx_fft_fixed_q15(&paramsCmplxFFT, input);
	...
	msp_split_q15(&paramsSplit, input);
