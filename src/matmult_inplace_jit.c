...
int _jit_safe_0(msp_matrix_mpy_q15_params * params, int * src1, int * src2)  {
;
...
void safe_matmult(msp_matrix_mpy_q15_params* params, int* src1, int* src2) {
	undoLogPtr_bak = undoLogPtr;
	undoLogCnt_bak = undoLogCnt;
	PROTECT_BEGIN();
	undoLog(src1,  S1_RS*S1_CS*2);
	PROTECT_END_NOWARCLR();
	PROTECT_BEGIN();
	_jit_safe_0(params, src1, src2);
	PROTECT_END();
	;
}
/*
...
#else
			safe_matmult(&mpyParams, *src_nv, *src_nv);
#endif
		}
