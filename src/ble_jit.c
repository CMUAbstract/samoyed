...
int _jit_safe_0(uint8_t * payload, size_t size)  {
...
void safe_send_radio(uint8_t* payload, size_t size) {
	...
	undoLogPtr = undoLogPtr_tmp;
	undoLogCnt = undoLogCnt_tmp;
	success = _jit_safe_0(payload, size);
	PROTECT_END();
	if (!success) {
		;
...
int main()
{
	...
	P1OUT &= ~BIT3;
	P1DIR |= BIT3;
	char _jit_test_payload[ 1];
	fill_with_rand(_jit_test_payload,  1);
	...
	P1OUT |= BIT3;
	while (1) {
		safe_send_radio((uint8_t *)_jit_test_payload, 1);
